...
class Contact
{
  ...
  public function addRecord() {
    ...
    if ($this->_verifyInput()) {
      ...
      if (trim($_POST['password1'])) {
        $hash_password = sha1(trim($_POST['password1']));
      }
      ...
      VALUES ('" . Database::prep($this->first_name) . "',
       ...
       '" . Database::prep($this->user_name) . "',
       '" . $hash_password . "',
       '" . Database::prep($this->access) . "')";
      ...
      $return = array('contactmaint', 'No Contact Record Added. Missing required information
       or the passwords do not match or are too short.');
      return $return;
    }
  ...
  public function editRecord() {
    ...
    if ($this->_verifyInput()) {
      ...
      if (trim($_POST['password1'])) {
        ...
        $query = 'UPDATE `contacts` SET first_name=?, last_name=?, position=?, email=?, phone=?,
          user_name=?, password=?, access=? WHERE id=?';
        $statement = $connection->prepare($query);
        // bind the parameters
        $statement->bind_param('ssssssssi',$this->first_name, $this->last_name, $this->position, 
          $this->email, $this->phone, $this->user_name, $hash_password, $this->access, $this->id);
      
      } else {
        // update without a password changed
        // Set up the prepared statement
        $query = 'UPDATE `contacts` SET first_name=?, last_name=?, position=?, email=?, phone=?,
          user_name=?, access=? WHERE id=?';
        $statement = $connection->prepare($query);
        // bind the parameters
        $statement->bind_param('sssssssi',$this->first_name, $this->last_name, $this->position, 
          $this->email, $this->phone, $this->user_name, $this->access, $this->id);
      }
  ...
  public static function logIn($item) {
    ...
    $connection = Database::getConnection();
    // Set up the query
    $query = 'SELECT id, first_name, last_name, user_name, access FROM `contacts` 
      WHERE user_name="'. $item['user_name'].'" AND password = "' . sha1($item['password']) . '" LIMIT 1';
    // Run the MySQL command   
    $result_obj = '';  
    // Run the MySQL command
    $result_obj = $connection->query($query);  
